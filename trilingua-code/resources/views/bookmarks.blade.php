...
@section('styles')
    ...
    {{-- Inline modal styles (same as Saved Translations) --}}
    <style>
...
<div class="stack">
    ...
    @if ($error)
        <p class="error-message">Unable to load bookmarks. Please try again later.</p>
    @else

    {{-- Page header --}}
    <div class="history-page-header">
        <div class="history-page-header__title">
            <h2>Bookmarked <span class="count-badge">{{ count($records) }}</span></h2>
        </div>
        <div class="history-page-header__controls">
            <input type="search" id="history-search" class="history-search" placeholder="Search bookmarks…" aria-label="Search bookmarks">
    ...
    @if (empty($records))
        <p class="empty-state">You have no bookmarked translations yet. Use the bookmark icon on a translation to save it here.</p>
    @else
                    ...
                    <div class="history-card__footer">
                        ...
                        <div class="history-card__actions">
                            <button class="history-card__action-btn view-details-btn"
                                    title="View details" data-id="{{ $record['id'] }}" aria-label="View translation details">
                            ...
                            <button class="history-card__action-btn redownload-btn"
                                    title="Re-download" data-id="{{ $record['id'] }}" data-filename="{{ $record['translated_filename'] ?? '' }}" aria-label="Re-download document">
                            ...
                            <button class="history-card__action-btn bookmark-btn bookmark-btn--active"
                                    title="Remove bookmark" data-id="{{ $record['id'] }}" aria-label="Remove bookmark">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>
                            </button>
                        </div>
                    </div>
                </div>
    ...
    <div class="detail-modal__dialog" role="dialog" aria-modal="true" aria-labelledby="detail-modal-title">
        ...
        <div id="detail-modal-content" style="display:none">
            ...
            <div class="detail-modal__actions">
                <button class="detail-modal__action-btn detail-modal__action-btn--primary" id="detail-action-view">View Translation</button>
                ...
                <button class="detail-modal__action-btn" id="detail-action-unbookmark">Remove Bookmark</button>
            </div>
        </div>
    </div>
</div>
...
(function () {
    ...
    function redownload(id) {
        clearError();
        return fetchJson('/history/redownload/' + encodeURIComponent(id), {
            method: 'POST',
    ...
    document.querySelectorAll('.redownload-btn').forEach(function (btn) {
        btn.addEventListener('click', function () {
            redownload(btn.getAttribute('data-id'))
                .then(function (data) {
    ...
    // ── Unbookmark (card buttons + detail modal) ─────────────────────────────
    function toggleBookmark(id, button) {
        return fetchJson('/history/' + encodeURIComponent(id) + '/bookmark', {
            method: 'POST',
            headers: { 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json', 'Content-Type': 'application/json' }
        }).then(function (data) {
            if (data && data.success) {
                if (!data.value) {
                    if (window.showToast) showToast('success', 'Removed', 'Bookmark removed.');
                    if (button) {
    ...
    function openDetailModal() { detailModal.style.display = 'flex'; document.body.style.overflow = 'hidden'; }
    function closeDetailModal() { detailModal.style.display = 'none'; document.body.style.overflow = ''; currentRecordId = null; }
    ...
    function renderDetail(data) {
        ...
        viewBtn.onclick = function () {
            if (isDoc) {
                redownload(data.id).then(function (res) { if (res && res.download_url) window.location.href = res.download_url; });
            } else if (navigator.clipboard && data.translated_text) {
        ...
        var unbookmarkBtn = document.getElementById('detail-action-unbookmark');
        unbookmarkBtn.onclick = function () {
            toggleBookmark(currentRecordId, null).then(function () {
                closeDetailModal();
                var card = document.querySelector('.history-card[data-id="' + currentRecordId + '"]');
                if (card) card.remove();
            });
        };
    }
    ...
    if (detailModal) detailModal.querySelector('.detail-modal__overlay').addEventListener('click', closeDetailModal);
    ...
    function closeDeleteConfirm() { deleteConfirmModal.style.display = 'none'; deleteTargetId = null; }
    ...
    if (contentEl) {
        function getAllCards() { return Array.from(contentEl.querySelectorAll('.history-card')); }
        function getAllGroups() { return Array.from(contentEl.querySelectorAll('.history-group')); }
        ...
        function applyGroupBy(mode) {
            getAllGroups().forEach(function (g) {
                ...
                if (heading) heading.style.display = (mode === 'none') ? 'none' : '';
            });
        }
    ...
    document.addEventListener('keydown', function (e) {
        if (e.key === 'Escape') { closeDetailModal(); closeDeleteConfirm(); }
    });
})();
</script>
@endsection
